defmodule SymphonyElixir.FailurePathTest do
  ...
  describe "verify failure and exhaustion" do
    setup do
      ...
      System.cmd("git", ["-c", "user.name=Test", "-c", "user.email=t@t.com", "commit", "-m", "init"], cd: workspace)
    ...
    test "verify_error is cleared by verify-fix closeout when HEAD advanced",
      ...
      System.cmd("git", ["-c", "user.name=Test", "-c", "user.email=t@t.com", "commit", "-m", "verify-fix"], cd: ws)
  ...
  describe "circuit breaker fires within bounded attempts" do
    ...
    test "replay at attempt 1 dispatches normally" do
      exec = %{@default_exec | "current_unit" => %{"kind" => "implement_subtask", "subtask_id" => "plan-1", "attempt" => 1}}
    ...
    test "replay at attempt 2 dispatches normally" do
      exec = %{@default_exec | "current_unit" => %{"kind" => "implement_subtask", "subtask_id" => "plan-1", "attempt" => 2}}
      ctx = %{issue: %{state: "In Progress"}, exec: exec, workpad_text: nil, git_head: "abc"}
      assert {:dispatch, %Unit{attempt: 3}} = DispatchResolver.resolve(ctx)
    end

    test "replay at max attempt triggers circuit breaker" do
      exec = %{@default_exec | "current_unit" => %{"kind" => "implement_subtask", "subtask_id" => "plan-1", "attempt" => 3}}
      ctx = %{issue: %{state: "In Progress"}, exec: exec, workpad_text: nil, git_head: "abc"}
